...
func TestLogger(t *testing.T) {
	logger, err := NewFluentLogger("tcp://127.0.0.1:24224")
	if err != nil {
		t.Error(err)
	}
	defer logger.Close()
...
func TestLoggerWithOpt(t *testing.T) {
	var duration time.Duration = 1000000000
	logger, err := NewFluentLogger("tcp://127.0.0.1:24224", WithTimeout(duration))
	if err != nil {
		t.Error(err)
	}
	defer logger.Close()
...
func BenchmarkLoggerDirect(b *testing.B) {
	b.SetParallelism(100)
	logger, err := NewFluentLogger("tcp://127.0.0.1:24224")
	if err != nil {
		b.Error(err)
...
func BenchmarkLoggerInfow(b *testing.B) {
	b.SetParallelism(100)
	logger, err := NewFluentLogger("tcp://127.0.0.1:24224")
	if err != nil {
		b.Error(err)
